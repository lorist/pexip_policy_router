"""
Run: pytest -v policy_router/tests/test_participant_policy_matching.py
"""
import pytest
import json
...
from policy_router.views import proxy_participant_policy


@pytest.mark.django_db
class TestParticipantPolicySourceMatchBehavior:
    def make_request(self, local_alias="room-1", ip="10.0.0.10", forwarded_for=None):
        ...
        request = rf.get("/policy/v1/participant/properties", {
    ...
    def test_rule_without_source_does_not_block_ip_rule(self, db):
        ...
        PolicyProxyRule.objects.create(
            ...
            override_participant_response={"status": "success", "action": "matched-ip"},
        )

        request = self.make_request(local_alias="room-123", ip="10.0.0.10")
        response = proxy_participant_policy(request)
    ...
    def test_source_match_normalization_variants(self, db):
        ...
        for variant in ["", " ", "None", "null", "NULL", " none "]:
            rule = PolicyProxyRule.objects.create(
                ...
                call_directions=["in"],
                always_continue_participant=True,
                override_participant_response={"action": "normalized"},
            )
    ...
    def test_forwarded_for_ip_matching(self, db):
        ...
        PolicyProxyRule.objects.create(
            ...
            call_directions=["in"],
            always_continue_participant=True,
            override_participant_response={"action": "via-forwarded"},
        )
    ...
    def test_duplicate_regex_with_distinct_sources_allowed(self, db):
        ...
        PolicyProxyRule.objects.create(
            ...
            call_directions=["in"],
            always_continue_participant=True,
            override_participant_response={"action": "a"},
        )
        ...
        PolicyProxyRule.objects.create(
            ...
            call_directions=["in"],
            always_continue_participant=True,
            override_participant_response={"action": "b"},
        )
    ...
    def test_identical_regex_and_source_raises(self, db):
        ...
        PolicyProxyRule.objects.create(
            ...
            call_directions=["in"],
            always_continue_participant=True,
            override_participant_response={"action": "ok"},
        )
        with pytest.raises(Exception):
            PolicyProxyRule.objects.create(
                ...
                call_directions=["in"],
                always_continue_participant=True,
                override_participant_response={"action": "fail"},
            )
